...
describe('transaction and message signing [ @E2E ]', function() {
  ...
  const basicOptions = {data : Basic.bytecode, gasPrice : '1', gas : 4000000};
  ...
  it('sendSignedTransaction (accounts.signTransaction with signing options)',
     async function() {
  ...
  it('sendSignedTransaction (accounts.signTransaction / without signing options)',
     async function() {
       const source = wallet[0].address;
       const destination = wallet[1].address;
  ...
  it('accounts.signTransaction errors when common, chain and hardfork all defined',
     async function() {
       const source = wallet[0].address;
       const destination = wallet[1].address;
       ...
       try {
         await web3.eth.accounts.signTransaction(txObject,
                                                 wallet[0].privateKey);
         assert.fail()
       } catch (err) {
         assert(
             err.message.includes('common object or the chain and hardfork'));
       }
     });

  it('accounts.signTransaction errors when chain specified without hardfork',
     async function() {
       const source = wallet[0].address;
       const destination = wallet[1].address;
  ...
  it('accounts.signTransaction errors when hardfork specified without chain',
     async function() {
       const source = wallet[0].address;
       const destination = wallet[1].address;
  ...
  it('accounts.signTransaction errors when tx signing is invalid',
     async function() {
       const source = wallet[0].address;
       const destination = wallet[1].address;
  ...
  it('accounts.signTransaction errors when no transaction is passed',
     async function() {
       try {
         await web3.eth.accounts.signTransaction(undefined,
                                                 wallet[0].privateKey);
         assert.fail()
       } catch (err) {
  ...
  it('transactions sent with wallet throws error correctly (with receipt)',
     async function() {
  ...
  it('eth.accounts.sign', async function() {
    ...
    const recovered =
        await web3.eth.personal.ecRecover(message, signed.signature);
