...
describe('method.call [ @E2E ]', function() {
  ...
  var miscOptions = {data : Misc.bytecode, gasPrice : '1', gas : 4000000};
  ...
  describe('http', function() {
    ...
    it('retrieves a uint value', async function() {
      ...
      await instance.methods.setValue(expected).send({from : accounts[0]});

      var value = await instance.methods.getValue().call({from : accounts[0]});
  ...
  describe('revert handling', function() {
    ...
    it('returns the expected revert reason string', async function() {
      try {
        await instance.methods.reverts().call({from : accounts[0]});
