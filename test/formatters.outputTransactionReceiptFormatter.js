...
describe('outputTransactionReceiptFormatter', function() {
  ...
  it('call outputTransactionReceiptFormatter when status is "0x1"', function() {
    ...
    assert.equal(formatters.outputTransactionReceiptFormatter(receipt).status,
                 true)
  });

  it('call outputTransactionReceiptFormatter when status is "0x01"',
     function() {
       var receipt = {
         status : '0x01',
         cumulativeGasUsed : '0x100',
       ...
       assert.equal(
           formatters.outputTransactionReceiptFormatter(receipt).status, true)
     });

  it('call outputTransactionReceiptFormatter when status is "undefined"',
     function() {
       var receipt = {
         status : undefined,
         cumulativeGasUsed : '0x100',
       ...
       assert.equal(
           formatters.outputTransactionReceiptFormatter(receipt).status,
           undefined)
     });
  ...
  it('call outputTransactionReceiptFormatter when status is "null"',
     function() {
       ...
       assert.equal(
           formatters.outputTransactionReceiptFormatter(receipt).status, null)
     });
});
