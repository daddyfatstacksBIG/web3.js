...
describe('method.send [ @E2E ]', function() {
  ...
  var basicOptions = {data : Basic.bytecode, gasPrice : '1', gas : 4000000};
  ...
  describe('http', function() {
    ...
    it('returns a receipt', async function() {
      var receipt =
          await instance.methods.setValue('1').send({from : accounts[0]});
    ...
    it('errors on revert', async function() {
      try {
        await instance.methods.reverts().send({from : accounts[0]});
  ...
  describe('ws', function() {
    ...
    it('fires the confirmation handler', function() {
      return new Promise(async (resolve, reject) => {
        ...
        await instance.methods.setValue('1')
            .send({from : accounts[0]})
    ...
    it('fires the error handler on revert', function(done) {
      instance.methods.reverts().send({from : accounts[0]}).on('error', err => {
        assert(err.message.includes('revert'));
        done();
      });
    });
  ...
  describe('with revert handling activated', function() {
    ...
    it('Promise throws on failing require with a revert reason given',
       async function() {
         try {
    ...
    it('Promise throws on failing require without a revert reason given',
       async function() {
         try {
    ...
    it('fires the error handler on failing require without a revert reason given',
       function(done) {
         instance.methods.requireWithoutReason()
             .send({from : accounts[0]})
    ...
    it('fires the error handler on failing require with a revert reason given',
       function(done) {
         instance.methods.requireWithReason()
             .send({from : accounts[0]})
             .on('error', (err, receipt) => {
               assert.equal(receipt.status, false);
               assert.equal(err.signature, 'Error(String)');
