...
var cryp = (typeof global === 'undefined') ? require('crypto-browserify')
                                           : require('crypto');
...
var Accounts = function Accounts() {
  ...
  var _ethereumCall = [
    ...
    new Method({name : 'getGasPrice', call : 'eth_gasPrice', params : 0}),
    new Method({
...
Accounts.prototype._addAccountFunctions = function(account) {
  ...
  account.sign = function sign(
      data) { return _this.sign(data, account.privateKey); };

  account.encrypt = function encrypt(
      password,
      options) { return _this.encrypt(account.privateKey, password, options); };
...
Accounts.prototype.signTransaction = function signTransaction(tx, privateKey,
                                                              callback) {
  var _this = this, error = false, transactionOptions = {}, result,
  ...
  // Resolve immediately if nonce, chainId, price and signing options are
  // provided
  if (tx.nonce !== undefined && tx.chainId !== undefined &&
      tx.gasPrice !== undefined && hasTxSigningOptions) {
  ...
  return Promise
      .all([
        ...
        isNot(tx.nonce) ? _this._ethereumCall.getTransactionCount(
...
Accounts.prototype.recover = function recover(message, signature, preFixed) {
  ...
  if (_.isObject(message)) {
    return this.recover(
        message.messageHash,
        Account.encodeSignature([ message.v, message.r, message.s ]), true);
  }
  ...
  if (args.length >= 4) {
    ...
    return this.recover(message, Account.encodeSignature(args.slice(1, 4)),
                        preFixed); // v, r, s
  }
...
Accounts.prototype.decrypt = function(v3Keystore, password, nonStrict) {
  ...
  var decipher =
      cryp.createDecipheriv(json.crypto.cipher, derivedKey.slice(0, 16),
                            Buffer.from(json.crypto.cipherparams.iv, 'hex'));
  var seed =
      '0x' + Buffer.concat([ decipher.update(ciphertext), decipher.final() ])
                 .toString('hex');
...
Accounts.prototype.encrypt = function(privateKey, password, options) {
  ...
  var kdfparams = {dklen : options.dklen || 32, salt : salt.toString('hex')};
  ...
  var cipher = cryp.createCipheriv(options.cipher || 'aes-128-ctr',
...
Wallet.prototype._currentIndexes = function() {
  ...
  var indexes = keys.map(function(key) { return parseInt(key); })
...
Wallet.prototype.clear = function() {
  ...
  indexes.forEach(function(index) { _this.remove(index); });
...
Wallet.prototype.encrypt = function(password, options) {
  ...
  var accounts = indexes.map(function(
      index) { return _this[index].encrypt(password, options); });
...
Wallet.prototype.save = function(password, keyName) {
  localStorage.setItem(keyName || this.defaultKeyName,
 ...
 * @returns {Boolean} a boolean indicating whether the specified storage is
 *     available or not
 */
