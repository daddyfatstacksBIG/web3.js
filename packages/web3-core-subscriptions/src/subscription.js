...
Subscription.prototype._formatInput = function(args) {
  ...
  var formattedArgs = subscription.inputFormatter.map(function(
      formatter,
      index) { return formatter ? formatter(args[index]) : args[index]; });
...
Subscription.prototype._toPayload = function(args) {
  ...
  return {method : this.options.type + '_subscribe', params : params};
};
...
Subscription.prototype.subscribe = function() {
  ...
  if (!this.options.requestManager.provider.on) {
    var err2 =
        new Error('The current provider doesn\'t support subscriptions: ' +
  ...
  if (payload.params[0] === 'logs' && _.isObject(payload.params[1]) &&
      payload.params[1].hasOwnProperty('fromBlock') &&
      isFinite(payload.params[1].fromBlock)) {
    // send the subscription request
    this.options.requestManager.send(
        {method : 'eth_getLogs', params : [ payload.params[1] ]},
        function(err, logs) {
          if (!err) {
  ...
  this.options.requestManager.send(payload, function(err, result) {
    if (!err && result) {
      ...
      _this.options.requestManager.addSubscription(
          _this.id, payload.params[0], _this.options.type,
          function(err, result) {
            if (!err) {
              if (!_.isArray(result)) {
                result = [ result ];
              ...
              result.forEach(function(resultItem) {
                ...
                if (_.isFunction(
                        _this.options.subscription.subscriptionHandler)) {
                ...
                // call the callback, last so that unsubscribe there won't
                // affect the emit above
                _this.callback(null, output, _this);
              });
            } else {
              ...
              // call the callback, last so that unsubscribe there won't affect
              // the emit above
              _this.callback(err, null, _this);
            }
          });
