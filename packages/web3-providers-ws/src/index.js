...
var isNode = Object.prototype.toString.call(typeof process !== 'undefined'
                                                ? process
                                                : 0) === '[object process]';
...
var parseURL = null;
if (isNode) {
  _btoa = function(str) { return Buffer.from(str).toString('base64'); };
  var url = require('url');
  if (url.URL) {
    // Use the new Node 6+ API for parsing URLs that supports username/password
    var newURL = url.URL;
    parseURL = function(url) { return new newURL(url); };
  } else {
  ...
  _btoa = btoa;
  parseURL = function(url) { return new URL(url); };
}
...
var WebsocketProvider = function WebsocketProvider(url, options) {
  ...
  this.connection =
      new Ws(url, protocol, undefined, headers, requestOptions, clientConfig);
...
WebsocketProvider.prototype.addDefaultEvents = function() {
  ...
  this.connection.onerror = function() { _this._timeout(); };
...
WebsocketProvider.prototype.send = function(payload, callback) {
  ...
  if (this.connection.readyState === this.connection.CONNECTING) {
    setTimeout(function() { _this.send(payload, callback); }, 10);
    return;
  }
 ...
 * @returns {boolean}
 */
WebsocketProvider.prototype.supportsSubscriptions = function() { return true; };
