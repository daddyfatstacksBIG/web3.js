...
var Contract = function Contract(jsonInterface, address, options) {
  ...
  if (_.isObject(lastArg) && !_.isArray(lastArg)) {
    ...
    this.options =
        _.extend(this.options, this._getOrSetDefaultOptions(options));
  ...
  Object.defineProperty(this.options, 'address', {
    set : function(value) {
      if (value) {
        _this._address =
            utils.toChecksumAddress(formatters.inputAddressFormatter(value));
      }
    },
    get : function() { return _this._address; },
    enumerable : true
  });
  ...
  Object.defineProperty(this.options, 'jsonInterface', {
      ...
      _this._jsonInterface = value.map(function(method) {
        ...
        method.constant =
            (method.stateMutability === "view" ||
             method.stateMutability === "pure" || method.constant);
        method.payable =
        ...
        if (method.type === 'function') {
          ...
          func = _this._createTxObject.bind({method : method, parent : _this});
    ...
    get : function() { return _this._jsonInterface; },
    enumerable : true
  });
  ...
  Object.defineProperty(this, 'handleRevert', {
    get : function() {
      if (_this.options.handleRevert === false ||
    ...
    set : function(val) { _this.options.handleRevert = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'defaultCommon', {
    ...
    set : function(val) { _this.options.common = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'defaultHardfork', {
    ...
    set : function(val) { _this.options.hardfork = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'defaultChain', {
    ...
    set : function(val) { _this.options.chain = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'transactionPollingTimeout', {
    ...
    set : function(val) { _this.options.transactionPollingTimeout = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'transactionConfirmationBlocks', {
    ...
    set : function(val) { _this.options.transactionConfirmationBlocks = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'transactionBlockTimeout', {
    ...
    set : function(val) { _this.options.transactionBlockTimeout = val; },
    enumerable : true
  });
  Object.defineProperty(this, 'defaultAccount', {
    get : function() { return defaultAccount; },
    set : function(val) {
      if (val) {
        defaultAccount =
            utils.toChecksumAddress(formatters.inputAddressFormatter(val));
      }
...
Contract.prototype._checkListener = function(type, event) {
  if (event === type) {
    throw new Error('The event "' + type +
...
Contract.prototype._getOrSetDefaultOptions = function getOrSetDefaultOptions(
  ...
  var from = options.from ? utils.toChecksumAddress(
...
Contract.prototype._encodeEventABI = function(event, options) {
  ...
  ['fromBlock', 'toBlock']
      ...
      .forEach(function(
          f) { result[f] = formatters.inputBlockNumberFormatter(options[f]); });
  ...
  if (_.isArray(options.topics)) {
    ...
    if (event.name !== 'ALLEVENTS') {
      var indexedTopics =
          event.inputs.filter(function(i) { return i.indexed === true; })
              .map(function(i) {
                ...
                if (_.isArray(value)) {
                  return value.map(function(
                      v) { return abi.encodeParameter(i.type, v); });
                }
...
Contract.prototype._decodeEventABI = function(data) {
  ...
  if (event.name === 'ALLEVENTS') {
    event = event.jsonInterface.find(function(
                intf) { return (intf.signature === data.topics[0]); }) ||
            {anonymous : true};
  }

  // create empty inputs if none are present (e.g. anonymous events on
  // allEvents)
  ...
  result.raw = {data : result.data, topics : result.topics};
  delete result.data;
  delete result.topics;
  ...
  var signature = false,
      paramsABI =
          this._parent.options.jsonInterface
              ...
              .map(function(json) {
                var inputLength =
                    (_.isArray(json.inputs)) ? json.inputs.length : 0;
...
/**
 * Deploys a contract and fire events based on its state: transactionHash,
 * receipt
 *
 * All event listeners will be removed, once the last possible event is fired
 * ("error", or "receipt")
 *
 * @method deploy
 * @param {Object} options
 * @param {Function} callback
 * @return {Object} EventEmitter possible events are "error", "transactionHash"
 *     and "receipt"
 */
Contract.prototype.deploy = function(options, callback) {
  ...
  var constructor =
      _.find(this.options.jsonInterface,
             function(method) { return (method.type === 'constructor'); }) ||
      {};
...
Contract.prototype._generateEventOptions = function() {
  ...
  if (!event) {
    throw new Error('Event "' + eventName +
...
/**
 * Adds event listeners and creates a subscription, and remove it once its
 * fired.
 *
 * @method clone
...
Contract.prototype.clone = function() {
  return new this.constructor(this.options.jsonInterface, this.options.address,
                              this.options);
};
...
Contract.prototype._on = function() {
  ...
  // TODO check if listener already exists? and reuse subscription if options
  // are the same.
  ...
  subscription.subscribe('logs', subOptions.params,
...
Contract.prototype._createTxObject = function _createTxObject() {
  ...
  if (this.method.type === 'function') {
    ...
    txObject.call.request = this.parent._executeMethod.bind(
        txObject, 'call', true); // to make batch requests
  }

  txObject.send = this.parent._executeMethod.bind(txObject, 'send');
  txObject.send.request = this.parent._executeMethod.bind(
      txObject, 'send', true); // to make batch requests
  ...
  if (args && this.method.inputs && args.length !== this.method.inputs.length) {
    ...
    throw errors.InvalidNumberOfParams(args.length, this.method.inputs.length,
                                       this.method.name);
  }
...
Contract.prototype._processExecuteArguments = function _processExecuteArguments(
  ...
  if (processedArgs.type === 'call' && args[args.length - 1] !== true &&
  ...
  processedArgs.options =
      this._parent._getOrSetDefaultOptions(processedArgs.options);
  ...
  if (!processedArgs.options.data)
    return utils._fireError(
        ...
        defer.eventEmitter, defer.reject, processedArgs.callback);
 ...
 * @param {Boolean} makeRequest if true, it simply returns the request
 *     parameters, rather than executing it
 */
Contract.prototype._executeMethod = function _executeMethod() {
  var _this = this,
      args = this._parent._processExecuteArguments.call(
          this, Array.prototype.slice.call(arguments), defer),
  ...
  if (args.generateRequest) {
    ...
    if (args.type === 'call') {
      payload.params.push(formatters.inputDefaultBlockNumberFormatter.call(
          this._parent, args.defaultBlock));
      payload.method = 'eth_call';
      payload.format =
          this._parent._decodeMethodReturn.bind(null, this._method.outputs);
    } else {
    ...
    case 'estimate':
      ...
      var estimateGas =
          (new Method({
            ...
            accounts :
                ethAccounts, // is eth.accounts (necessary for wallet signing)
    ...
    case 'call':
      ...
      // TODO check errors: missing "from" should give error on deploy and send,
      // call ?
      ...
      var call =
          (new Method({
            ...
            outputFormatter : function(result) {
              return _this._parent._decodeMethodReturn(_this._method.outputs,
                                                       result);
            },
            requestManager : _this._parent._requestManager,
            accounts :
    ...
    case 'send':
      ...
      if (!utils.isAddress(args.options.from)) {
        return utils._fireError(
            ...
            defer.eventEmitter, defer.reject, args.callback);
      }

      if (_.isBoolean(this._method.payable) && !this._method.payable &&
          args.options.value && args.options.value > 0) {
        return utils._fireError(
            new Error(
      ...
      var sendTransaction =
          (new Method({
            ...
            requestManager : _this._parent._requestManager,
            accounts : _this.constructor._ethAccounts ||
                           _this._ethAccounts, // is eth.accounts (necessary for
                                               // wallet signing)
            defaultAccount : _this._parent.defaultAccount,
            defaultBlock : _this._parent.defaultBlock,
