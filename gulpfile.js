...
gulp.task(
    'clean',
    gulp.series('lint',
                function(cb) { del([ DEST ]).then(cb.bind(null, null)); }));
...
packages.forEach(function(pckg, i) {
  ...
  gulp.task(pckg.fileName, gulp.series(prevPckg, function() {
    ...
    if (pckg.ignore) {
      pckg.ignore.forEach(function(ignore) { stream.ignore(ignore); });
    }

    var bundle = stream.bundle();

    stream = bundle.pipe(exorcist(path.join(DEST, pckg.fileName + '.js.map')));

    if (pckg.fileName === 'web3') {
      bundle.pipe(
          exorcist(path.join(WEB3_PACKAGE_DEST, pckg.fileName + '.js.map')));
    }

    stream = stream.pipe(source(pckg.fileName + '.js')).pipe(streamify(babel({
      compact : false,
      presets :
...
gulp.task('publishTag', function() {
  exec('git commit -am "add tag v' + lernaJSON.version + '"; git tag v' +
       lernaJSON.version + '; git push origin v' + lernaJSON.version + ';');
});
...
gulp.task('all', gulp.series('version', 'lint', 'clean',
